...
<script type="text/javascript">function isWeight(){google.charts.load('current', {'packages':['line']});google.charts.setOnLoadCallback(drawChart);function drawChart(){var data=google.visualization.arrayToDataTable([['Дата','Вес'],<#list weights?sortBy("data") as weight><#if weight.authorName==name>['${weight.data}',${weight.value}],</#if></#list>]);var options={curveType:'function',legend:{position:'bottom'}};var chart=new google.charts.Line(document.getElementById('weight_chart'));chart.draw(data,google.charts.Line.convertOptions(options))}}</script>
</@c.page>
